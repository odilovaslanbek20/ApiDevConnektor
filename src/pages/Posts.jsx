...
function Posts() {
	...
	const { data: posts, loading, error } = useGetData('posts')

	const postsArr = posts ? Object.values(posts) : []

	if (loading) {
		return (
			<div className='flex items-center justify-center flex-col mt-[100px] gap-[20px] text-[#17a2b8]'>
	...
	if (error) {
		return (
			<div className='text-red-500 mt-[100px] text-center p-4'>
				Error: {error}
			</div>
		)
	}
